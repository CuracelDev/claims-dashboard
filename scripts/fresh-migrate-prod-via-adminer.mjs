...
const TABLE_COLUMNS = {
  ...
  piles_auto_assignment_external_assignments: [
    ...
    ['owner_name', 'text'], ['first_detected_at', 'timestamptz'], ['last_seen_at', 'timestamptz'], ['notification_sent_at', 'timestamptz'],
    ['cleared_at', 'timestamptz'], ['is_active', 'boolean'], ['details', 'jsonb'], ['created_at', 'timestamptz'], ['updated_at', 'timestamptz'],
  ],
  piles_auto_assignment_logs: [
    ['id', 'text'], ['master_account_id', 'text'], ['bot_account_id', 'text'], ['insurer_name', 'text'],
...
const CREATE_TABLE_SQL = {
  ...
  piles_auto_assignment_external_assignments: `
    CREATE TABLE piles_auto_assignment_external_assignments (
      id text PRIMARY KEY DEFAULT md5(random()::text || clock_timestamp()::text),
      ...
      created_at timestamptz DEFAULT now(),
      updated_at timestamptz DEFAULT now(),
      CONSTRAINT piles_auto_assignment_external_assignments_unique UNIQUE (insurer_name, tracking_key)
    )`,
  piles_auto_assignment_logs: `
...
const INDEX_SQL = [
  ...
  'CREATE INDEX piles_auto_assignment_external_assignments_insurer_active_idx ON piles_auto_assignment_external_assignments (insurer_name, is_active, last_seen_at DESC)',
