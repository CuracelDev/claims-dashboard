...
export async function POST(request) {
  try {
    ...
    const summaryLines = (aggregations.by_issue || [])
      .slice(0, 8).map(r => `- ${r.issue}: ${r.count}`).join('\n');

    const insurerLines = (aggregations.by_insurer || [])
      .slice(0, 5).map(r => `- ${r.insurer}: ${r.count} flags`).join('\n');
    ...
    const instruction = detail === 'detailed'
      ? 'Write a detailed QA intelligence report (6-8 sentences). Cover: (1) overall flag volume, (2) top 2-3 issue types with counts and percentages, (3) most affected insurers or providers, (4) patterns suggesting systemic issues, (5) risk assessment, (6) two specific recommended actions. No markdown. No bullets. Plain prose only.'
      : 'Write a concise QA intelligence brief (3-5 sentences). Lead with the most significant finding. Identify patterns suggesting systemic issues. End with one specific actionable next step. Be direct, factual, use precise numbers. No markdown. No bullets. Plain prose only.';
...
${instruction}`;
    ...
    if (send_to_slack) {
      ...
      const slackResult = await slackPost(
        ...
        [
          { type: 'header', text: { type: 'plain_text', text: 'QA Intelligence Brief', emoji: true } },
          { type: 'context', elements: [{ type: 'mrkdwn', text: `*${dateStr}*  |  ${total} total flags` }] },
          { type: 'divider' },
          { type: 'section', text: { type: 'mrkdwn', text: insight } },
          { type: 'divider' },
          { type: 'context', elements: [{ type: 'mrkdwn', text: `AI-generated by Claims Intel | <https://claims-dashboard.vercel.app/qa|View QA Dashboard>` }] },
        ]
      );
      ...
      return Response.json({ insight, slack_sent: true, slack_ts: slackResult.ts, slack_channel: qaChannel });
    }
