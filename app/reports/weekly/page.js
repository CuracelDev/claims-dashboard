...
function getWeekBounds(dateStr) {
  ...
  return [toLocalYMD(mon), toLocalYMD(sun)];
}

function lastWeekBounds() {
  const today = getTodayLocal();
  return getWeekBounds(addDays(today, -7));
}
...
const QUICK = [
  { label: 'This week', fn: () => getWeekBounds(getTodayLocal()) },
  ...
  },
  {
    ...
    fn: () => {
      const now = new Date();
      const first = toLocalYMD(new Date(now.getFullYear(), now.getMonth(), 1, 12, 0, 0, 0));
      return [first, getTodayLocal()];
    },
  },
];
...
export default function WeeklyPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reports/weekly?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`)
      .then(async (r) => {
        const d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || 'Failed to load weekly summary');
        return d;
      })
      .then((d) => setData(d))
      ...
      <div style={{ padding: '20px 24px', boxSizing: 'border-box' }}>
        ...
        {!loading && data && data.total_reports > 0 && (
          <>
            ...
            >
              <div>
                <div style={{ fontWeight: 700, fontSize: 16, color: '#0B0F1A' }}>
                  {fmt(from)} – {fmt(to)}
                </div>
              ...
              <div style={{ display: 'flex', gap: 20 }}>
                {members.map((p) => (
                  <div key={p.person?.id} style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: '#0B0F1A' }}>
                      {p.days_reported}
                    </div>
                    <div style={{ fontSize: 11, color: '#0B0F1A88' }}>
                    ...
                    {top.map(([key, val]) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ color: C.text, fontWeight: 700, fontFamily: 'monospace', marginLeft: 8 }}>
                            {val.toLocaleString()}
                          </span>
                        </div>
                      );
