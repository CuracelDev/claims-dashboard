...
export default function BatchSplitterPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/tools/batch-splitter', { method: 'POST', body: formData });
      ...
      const batchCount = res.headers.get('X-Batch-Count');
      setResult({ url, totalRows, batchCount, fileName: file.name });
      setStatus('done');
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, padding: '32px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: C.sub, fontSize: '14px', margin: 0 }}>Split large CSV or Excel files into smaller operational batches and download as a ZIP.</p>
      </div>
      <div style={{ maxWidth: '640px' }}>
        <div style={{ marginBottom: '20px' }}>
          ...
          <div onClick={() => fileRef.current?.click()} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}
            ...
            {file ? (
              ...
              <div style={{ color: C.sub, fontSize: '12px', marginTop: '4px' }}>Supports .csv, .xlsx, .xls</div></>
            )}
          </div>
        </div>
        <div style={{ marginBottom: '24px' }}>
          ...
          <input type="number" min="100" max="100000" value={batchSize} onChange={e => setBatchSize(Number(e.target.value))}
        ...
        {status !== 'done' && (
          <button onClick={handleSubmit} disabled={!file || status === 'loading'}
            ...
            {status === 'loading' ? '⏳ Processing...' : '⚡ Split File'}
          </button>
        )}
        {status === 'done' && result && (
          <div style={{ background: '#00E5A010', border: '1px solid #00E5A040', borderRadius: '12px', padding: '24px', marginTop: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              <a href={result.url} download={`${result.fileName.replace(/\.[^.]+$/, '')}_batches.zip`}
