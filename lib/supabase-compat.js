...
class SupabaseQueryBuilder {
  ...
  gt(column, value) {
    this.conditions.push({ column, value, type: 'gt' });
    return this;
  }

  lt(column, value) {
    this.conditions.push({ column, value, type: 'lt' });
    return this;
  }
    ...
    try {
      ...
      const buildWhereClause = () => {
        ...
        const clauses = this.conditions.map((c) => {
          values.push(c.type === 'in' ? c.value : normalizeParamValue(c.value));
          ...
          if (c.type === 'gt') return `${c.column} > ${placeholder}`;
          if (c.type === 'lt') return `${c.column} < ${placeholder}`;
      ...
      if (this.mode === 'insert') {
        ...
        const placeholders = rows.map((row) => {
          const rowPlaceholders = keys.map((key) => {
            values.push(normalizeParamValue(row[key]));
        ...
        const setClause = keys.map((key) => {
          values.push(normalizeParamValue(this.payload[key]));
...
export function createClient() {
  return {
    ...
    rpc: async (fnName, params = {}) => {
      try {
        if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(String(fnName || ''))) {
          throw new Error(`Invalid RPC function name: ${fnName}`);
        }

        const keys = Object.keys(params || {});
        const values = [];
        const namedArgs = keys.map((key) => {
          if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(key)) {
            throw new Error(`Invalid RPC parameter key name: ${key}`);
          }
          let val = params[key];
        ...
        const sql = keys.length
          ? `SELECT * FROM "${fnName}"(${namedArgs.join(', ')})`
          : `SELECT * FROM "${fnName}"()`;
