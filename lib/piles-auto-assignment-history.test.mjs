...
test('bot timing controls state the fresh-versus-reassignment contract', () => {
  assert.match(pageSource, /fresh unassigned piles can be assigned on every scheduled run/i);
  assert.match(pageSource, /Reassignment can begin exactly at `Reassign from`/);
  assert.match(pageSource, /Reassignment Window/);
  assert.doesNotMatch(pageSource, /Grace mins before reassign|grace window has passed/);
  assert.doesNotMatch(pageSource, /shift_grace_minutes/);
});
