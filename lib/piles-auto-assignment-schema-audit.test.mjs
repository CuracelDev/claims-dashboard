...
function durableWorkAuditFixture() {
  const columns = [
    ...
    ['portal_environment', 'text', 'NO'], ['months', 'jsonb', 'NO'], ['year', 'text', 'NO'],
  ...
  const checks = [
    ...
    { columns: ['portal_environment'], definition: "CHECK ((portal_environment = ANY (ARRAY['production'::text, 'test'::text])))" },
    ...
    { columns: ['months'], definition: `CHECK (((jsonb_typeof(months) = 'array'::text) AND (jsonb_array_length(months) > 0) AND (months <@ '["All", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]'::jsonb) AND ((NOT (months ? 'All'::text)) OR (months = '["All"]'::jsonb))))` },
    { columns: ['year'], definition: "CHECK (((year = 'All'::text) OR (year ~ '^20[0-9]{2}$'::text)))" },
  ];
  const indexes = [
    { indexname: 'piles_auto_assignment_work_items_queued_generation_idx', is_unique: true, columns: ['canonical_insurer_name', 'source', 'request_scope', 'portal_environment', 'months', 'year'], predicate: "(disposition = 'queued'::text)" },
    { indexname: 'piles_auto_assignment_work_items_follow_up_idx', is_unique: true, columns: ['canonical_insurer_name', 'portal_environment', 'months', 'year'], predicate: "(disposition = 'follow_up_queued'::text)" },
    { indexname: 'piles_auto_assignment_work_items_completed_scope_idx', is_unique: false, columns: ['canonical_insurer_name', 'portal_environment', 'year', 'finished_at', 'id'], predicate: "(disposition = 'completed'::text)" },
...
test('schema audit distinguishes an exact unvalidated enum constraint from a changed expression', () => {
  const columns = [
    { column_name: 'id', data_type: 'text' }, { column_name: 'status', data_type: 'text' },
    { column_name: 'created_at', data_type: 'timestamp with time zone' },
    { column_name: 'updated_at', data_type: 'timestamp with time zone' },
  ...
  const definition = `CHECK ((status = ANY (ARRAY[${values.map((value) => `'${value}'::text`).join(', ')}]))) NOT VALID`;

  const issues = evaluateTable('piles_auto_assignment_runner_runs', columns, [], [
    { columns: ['status'], definition, is_validated: false },
  ]);
  assert.deepEqual(issues, ['status CHECK constraint must be VALIDATED']);
  assert.deepEqual(evaluateTable('piles_auto_assignment_runner_runs', columns, [], [
    { columns: ['status'], definition, is_validated: true },
  ]), []);

  for (const changed of [
    definition.replace("'completed_with_issues'::text", "'completed_with_issue'::text"),
    definition.replace("'completed_with_issues'::text", "'COMPLETED_WITH_ISSUES'::text"),
    definition.replace('))) NOT VALID', ')) OR true) NOT VALID'),
  ]) {
    assert.match(evaluateTable('piles_auto_assignment_runner_runs', columns, [], [
      { columns: ['status'], definition: changed, is_validated: false },
    ]).join(';'), /status CHECK.*completed_with_issues/i);
  }
});
...
test('readiness treats stale unsubmitted plans as unsafe outside deployment mode', async () => {
  const { evaluateAttemptHealth } = await import('../scripts/audit-piles-auto-assignment-readiness.mjs');
  const incident = evaluateAttemptHealth({ pendingReconciliation: 2, staleUnsubmitted: 3 });
  assert.equal(incident.find((item) => item.name === 'stale unsubmitted assignment attempts').ok, false);
  assert.equal(incident.find((item) => item.name === 'pending reconciliation age').ok, false);

  const deployment = evaluateAttemptHealth(
    { pendingReconciliation: 2, staleUnsubmitted: 3 },
    { deploymentMode: true },
  );
  assert.equal(deployment.every((item) => item.ok), true);
  assert.equal(deployment.every((item) => item.severity === 'warning'), true);
...
test('deployment audit separates repair deployment from runtime incident gates', () => {
  ...
  assert.match(readiness, /evaluateAttemptHealth\([\s\S]*pendingReconciliation: pending\.rows\[0\]\.count/);
  assert.match(readiness, /staleUnsubmitted: unsubmitted\.rows\[0\]\.count/);
  assert.match(readiness, /\{ deploymentMode \}/);
});
...
test('stale insurer recovery checks fresh heartbeat and pending evidence before terminalizing', async () => {
  ...
  assert.equal(db.calls.at(-1).text, 'COMMIT');
  const evidence = db.calls.find(({ name }) => name === 'recovery-run-evidence');
  assert.match(evidence.text, /attempt\.insurer_run_id = run\.id/);
  assert.match(evidence.text, /batch\.insurer_run_id = run\.id/);
  assert.doesNotMatch(evidence.text, /canonicalRecoverySql|attempt\.insurer_name|batch\.insurer_name/);
...
test('parent repair recovers only a stale orphaned durable preview', async () => {
  const { runRecovery } = await import('../scripts/recover-piles-stale-runs.mjs');
  const args = ['--apply', '--confirmation', 'RECOVER_STALE_RUNS', '--parent-run-id', 'parent-1'];
  const base = parentRecoveryResponses();
  base['recovery-parent-lock'][0] = {
    id: 'parent-1', status: 'running',
    details: { preview_protocol: 'durable_preview_v1', preview_phase: 'scan', preview_claim_token: 'private' },
  };
  const stale = recoveryDatabase({ ...base,
    'recovery-preview-parent-lock': [{ acquired: true }],
    'recovery-preview-parent-evidence': [{ eligible: true }],
    'recovery-preview-parent-update': [{ id: 'parent-1' }],
  });
  assert.equal((await runRecovery(stale, args)).parent_runs_repaired, 1);
  const update = stale.calls.find(({ name }) => name === 'recovery-preview-parent-update');
  assert.ok(update);
  assert.doesNotMatch(JSON.stringify(stale.calls), /private/);
  assert.match(update.text, /preview_stale_recovered/);
  assert.equal(stale.calls.at(-1).text, 'COMMIT');

  const fresh = recoveryDatabase({ ...base, 'recovery-preview-parent-lock': [{ acquired: true }],
    'recovery-preview-parent-evidence': [{ eligible: false }] });
  assert.equal((await runRecovery(fresh, args)).blocked, 1);
  assert.ok(fresh.calls.every(({ text }) => !/^\s*UPDATE/i.test(text)));
  assert.equal(fresh.calls.at(-1).text, 'ROLLBACK');

  const live = recoveryDatabase({ ...base, 'recovery-preview-parent-lock': [{ acquired: false }] });
  assert.equal((await runRecovery(live, args)).blocked, 1);
  assert.ok(live.calls.every(({ text }) => !/^\s*UPDATE/i.test(text)));
  assert.ok(live.calls.every(({ name }) => name !== 'recovery-preview-parent-evidence'));
});
...
test('workflow keeps ownership token out of the runner preamble until the private event reader masks it', () => {
  const workflow = parseYamlWithRuby(readFileSync(new URL('../.github/workflows/piles-stale-run-recovery.yml', import.meta.url), 'utf8'));
  const job = workflow.jobs.recover;
  const validation = job.steps[0];
  assert.equal(typeof validation.run, 'string', 'Inputs must be validated before the SSH action sees them');
  const token = `private-token-${randomUUID()}`;
  const inputs = { action: 'recover', confirmation: 'RECOVER_STALE_RUNS', work_id: 'work-1', claim_token: token };
  // Model Actions' expression expansion and pre-script env/preamble logging.
  // Masking inside the script cannot retroactively redact this earlier output.
  const expand = (value) => JSON.stringify(value).replace(/\$\{\{\s*(?:github\.event\.)?inputs\.([a-z_]+)\s*\}\}/g, (_, key) => inputs[key] || '');
  const preamble = expand({ workflow_env: workflow.env, job_env: job.env, first_step: validation });
  assert.equal(preamble.includes(token), false, 'The token is exposed before the first step can register its mask');
  assert.deepEqual(workflow.env || {}, {});
  assert.deepEqual(job.env || {}, {});
  assert.deepEqual(validation.env || {}, {});
  assert.doesNotMatch(JSON.stringify(validation), /\$\{\{/,
    'The private first step must be generic code, not an interpolated input or event payload');
  const directory = mkdtempSync(join(tmpdir(), 'piles-private-event-'));
  const eventPath = join(directory, 'event.json'); const envPath = join(directory, 'github-env');
  const execute = (eventInputs) => {
    writeFileSync(eventPath, JSON.stringify({ inputs: eventInputs }));
    writeFileSync(envPath, '');
    const result = spawnSync('/bin/bash', ['-c', validation.run], {
      env: { PATH: process.env.PATH, GITHUB_EVENT_PATH: eventPath, GITHUB_ENV: envPath }, encoding: 'utf8', timeout: 5000,
    });
    return { ...result, exported: readFileSync(envPath, 'utf8') };
  };
  try {
    const masked = execute(inputs);
    assert.equal(masked.status, 0, masked.stderr);
    assert.equal(masked.stdout.trim(), `::add-mask::${token}`);
    assert.equal(masked.stderr, '');
    const exported = Object.fromEntries(masked.exported.trim().split('\n').map((line) => line.split('=')));
    assert.equal(exported.RECOVERY_CLAIM_TOKEN, token);
    assert.equal(exported.RECOVERY_WORK_ID, 'work-1');
    // The next step's environment/routing only exists after add-mask was emitted.
    const nextPreamble = JSON.stringify({ env: exported, step: job.steps[1] }).split(token).join('***');
    assert.equal(nextPreamble.includes(token), false);
    writeFileSync(join(directory, '.env'), 'DATABASE_URL=fixture-do-not-contact\n');
    writeFileSync(join(directory, 'node'), `#!${process.execPath}\nconsole.log(JSON.stringify(process.argv.slice(2)));\n`, { mode: 0o700 });
    const routed = spawnSync('/bin/sh', ['-c', job.steps[1].with.script.replace('cd ~/claims-dashboard', `cd '${directory}'`)], {
      env: { ...exported, PATH: directory }, encoding: 'utf8', timeout: 5000,
    });
    assert.equal(routed.status, 0, routed.stderr);
    assert.deepEqual(JSON.parse(routed.stdout).slice(-2), ['--claim-token', token]);
    assert.equal(routed.stdout.split(token).join('***').includes(token), false);
    for (const value of ["x'; echo injected", 'x\ny', 'a'.repeat(201), '$(echo injected)']) {
      const result = execute({ action: 'inspect', work_id: value });
      assert.equal(result.status, 1);
      assert.equal(result.stdout, '');
      assert.equal(result.exported, '');
      assert.doesNotMatch(result.stderr, /injected|x';/);
    }
    const malicious = execute({ ...inputs, claim_token: 'secret%\n::error::injected' });
    assert.equal(malicious.status, 1);
    assert.equal(malicious.stdout.trim(), '::add-mask::secret%25%0A::error::injected');
    assert.equal(malicious.exported, '');
    assert.doesNotMatch(malicious.stderr, /secret|injected/);
  } finally { rmSync(directory, { recursive: true }); }
});

// Opt-in upgrade gate. The caller must create a private disposable PostgreSQL
// cluster under this exact path family; the test intentionally has no network
// URL fallback because it resets the public schema.
if (process.env.PILES_SCHEMA_UPGRADE_TEST_SOCKET) {
  test('base schema upgrades idempotently to validated status constraints accepted by the real audit', async () => {
    assert.match(process.env.PILES_SCHEMA_UPGRADE_TEST_SOCKET, /^\/private\/tmp\/piles-schema-upgrade\.[A-Za-z0-9]+$/);
    const { default: pg } = await import('pg');
    const pool = new pg.Pool({
      host: process.env.PILES_SCHEMA_UPGRADE_TEST_SOCKET,
      port: 55435,
      database: 'postgres',
      user: process.env.USER,
    });
    try {
      const baseline = spawnSync('git', ['show', '99dc867:scripts/piles-auto-assignment-schema.sql'], {
        cwd: new URL('..', import.meta.url), encoding: 'utf8', maxBuffer: 10 * 1024 * 1024,
      });
      assert.equal(baseline.status, 0, baseline.stderr);
      const head = readFileSync(new URL('../scripts/piles-auto-assignment-schema.sql', import.meta.url), 'utf8');
      const resetAuditDependencies = () => pool.query(`
        DROP SCHEMA public CASCADE;
        CREATE SCHEMA public;
        CREATE TABLE audit_log (
          id text PRIMARY KEY, member_id text, details jsonb, created_at timestamptz
        );
        CREATE TABLE daily_reports (
          id text PRIMARY KEY, team_member_id text, report_date date, metrics jsonb,
          tasks_completed text, notes text, status text, created_at timestamptz,
          UNIQUE (team_member_id, report_date)
        );
        CREATE TABLE team_members (
          id text PRIMARY KEY, name text, display_name text, active boolean, is_active boolean
        );
      `);
      const assertValidatedAndAuditable = async () => {
        const constraints = await pool.query(`
          SELECT conname, convalidated, pg_get_constraintdef(oid) AS definition
          FROM pg_constraint
          WHERE conname IN (
            'piles_auto_assignment_runner_runs_status_check',
            'piles_auto_assignment_insurer_runs_status_check',
            'piles_auto_assignment_schedule_requests_status_check'
          )
          ORDER BY conname
        `);
        assert.equal(constraints.rowCount, 3);
        assert.ok(constraints.rows.every((row) => row.convalidated === true));
        assert.ok(constraints.rows.every((row) => !/NOT VALID\s*$/i.test(row.definition)));
        await auditDatabase(pool, { log() {} });
      };

      await resetAuditDependencies();
      await pool.query(baseline.stdout);
      await pool.query(`INSERT INTO piles_auto_assignment_runner_runs (id, status) VALUES ('old-parent', 'completed')`);
      await pool.query(`INSERT INTO piles_auto_assignment_insurer_runs (id, runner_run_id, insurer_name, status) VALUES ('old-child', 'old-parent', 'fixture', 'completed')`);
      await pool.query(`INSERT INTO piles_auto_assignment_schedule_requests (id, insurer_name, status) VALUES ('old-request', 'fixture', 'claimed')`);

      await pool.query(head);
      await pool.query(head);
      await assertValidatedAndAuditable();
...
if (process.env.PILES_RECOVERY_TEST_SOCKET) {
  test('guarded recovery executes real PostgreSQL predicates, CAS and row-wait races', async (t) => {
    ...
    const parent = async (status = 'started') => {
      const id = identity();
      await db.query(`INSERT INTO piles_auto_assignment_runner_runs (id, status, run_scope, run_source, mode, months, year, finished_at)
        VALUES ($1, $2, 'all-active', 'schedule', 'execute', '["All"]'::jsonb, 'All', CASE WHEN $2 <> 'started' THEN clock_timestamp() ELSE NULL END)`, [id, status]);
      return id;
    };
    ...
    try {
      ...
      await t.test('unresolved evidence from another run does not block exact ledger-only finalization', async () => {
        const insurer = identity();
        const historical = await run(await parent('completed'), insurer, 'completed');
        await batch(historical, insurer, 'reconciliation_pending');
        const owner = await parent();
        const stale = await run(owner, insurer);
        const result = await runRecovery(db, [
          '--apply', '--confirmation', 'RECOVER_STALE_RUNS', '--insurer-run-id', stale,
        ]);
        assert.equal(result.insurer_runs_recovered, 1);
        assert.equal((await db.query(
          'SELECT status FROM piles_auto_assignment_insurer_runs WHERE id = $1', [stale],
        )).rows[0].status, 'failed');
        assert.equal((await db.query(
          'SELECT status FROM piles_auto_assignment_batches WHERE insurer_run_id = $1', [historical],
      ...
      const allScope = { portal: 'production', months: ['All'], year: 'All' };
      for (const [label, requested, successor, eligible] of [
        ['test portal cannot cover production', allScope, { ...allScope, portal: 'test' }, false],
        ['May 2025 cannot cover All All', allScope, { ...allScope, months: ['May'], year: '2025' }, false],
        ['one year cannot cover every year', allScope, { ...allScope, year: '2025' }, false],
        ['one month cannot cover every month', allScope, { ...allScope, months: ['May'] }, false],
        ['exact All All coverage is sufficient', allScope, allScope, true],
        ['exact narrow coverage is sufficient', { ...allScope, months: ['May'], year: '2025' }, { ...allScope, months: ['May'], year: '2025' }, true],
        ['true All scope covers a narrower request', { ...allScope, months: ['May', 'Jun'], year: '2025' }, allScope, true],
        ['known full and short month names normalize with set coverage', { ...allScope, months: ['May', 'Jun'], year: '2025' }, { ...allScope, months: [' June ', ' May ', 'Jul'], year: ' 2025 ' }, true],
        ['missing requested month is not covered', { ...allScope, months: ['May', 'Jun'] }, { ...allScope, months: ['May'] }, false],
        ['unknown month array is not All', allScope, { ...allScope, months: ['Unknown'] }, false],
        ['empty months is not All', allScope, { ...allScope, months: [] }, false],
        ['null months is not All', allScope, { ...allScope, months: null }, false],
        ['scalar months is not an established encoding', allScope, { ...allScope, months: 'All' }, false],
        ['mixed All and unknown does not prove coverage', allScope, { ...allScope, months: ['All', 'Unknown'] }, false],
        ['null month member does not prove coverage', allScope, { ...allScope, months: ['All', null] }, false],
        ['missing successor year is not All', allScope, { ...allScope, year: null }, false],
        ['empty successor year is not All', allScope, { ...allScope, year: '' }, false],
        ['matching unknown years is not evidence', { ...allScope, year: 'Current' }, { ...allScope, year: 'Current' }, false],
        ['missing requested scope is not guessed', { ...allScope, months: [], year: null }, allScope, false],
        ['manual successor can cover an adequately scoped scheduled request', allScope, { ...allScope, source: 'manual' }, true],
        ['dry-run cannot supersede execute', allScope, { ...allScope, mode: 'dry-run' }, false],
      ]) {
        await t.test(`legacy scope: ${label}`, async () => {
          const owner = await parent('skipped_overlap'); const successorOwner = await parent('completed');
          for (const [id, scope] of [[owner, requested], [successorOwner, successor]]) {
            await db.query(`UPDATE piles_auto_assignment_runner_runs SET portal_environment = $2, months = $3::jsonb,
              year = $4, run_source = $5, mode = $6 WHERE id = $1`,
            [id, scope.portal, JSON.stringify(scope.months), scope.year, scope.source || 'schedule', scope.mode || 'execute']);
          }
          const insurer = identity(); const requestId = identity();
          await db.query(`INSERT INTO piles_auto_assignment_schedule_requests (id, insurer_name, requested_runner_run_id, requested_at)
            VALUES ($1, $2, $3, clock_timestamp() - interval '1 hour')`, [requestId, insurer, owner]);
          await run(successorOwner, insurer, 'completed');
          const inspected = await runLegacyAudit(db, ['--request-id', requestId]);
          const applied = await runLegacyAudit(db, ['--apply', '--confirmation', 'CANCEL_OBSOLETE_LEGACY_REQUESTS', '--request-id', requestId]);
          const status = (await db.query('SELECT status FROM piles_auto_assignment_schedule_requests WHERE id = $1', [requestId])).rows[0].status;
          assert.deepEqual({ candidates: inspected.obsolete_requests, cancelled: applied.cancelled_requests, status },
            { candidates: eligible ? 1 : 0, cancelled: eligible ? 1 : 0, status: eligible ? 'cancelled_legacy' : 'pending' });
        });
      }
    } finally {
...
test('incident report retains only allowlisted failure location metadata', async () => {
  const { buildIncidentReport } = await import('../scripts/inspect-piles-runner-incidents.mjs');
  const report = buildIncidentReport({ hours: 1, runId: null, workItemsAvailable: false,
    sections: { insurers: [
      { failure: { phase: 'reconcile', module: 'store', error_type: 'TypeError', line: 123, message: 'secret patient', locals: 'secret' } },
      { failure: { phase: '<secret>', module: 'private', error_type: ['secret'], line: true } },
    ] } });
  assert.deepEqual(report.insurers[0].failure, { phase: 'reconcile', module: 'store', error_type: 'TypeError', line: 123 });
  assert.deepEqual(report.insurers[1].failure, { phase: 'other', module: 'other', error_type: 'other', line: 0 });
  assert.equal(JSON.stringify(report).includes('secret'), false);
});
...
test('incident inspection configures production PostgreSQL SSL consistently with readiness audits', async () => {
  const { poolConfigForDatabaseUrl } = await import('../scripts/inspect-piles-runner-incidents.mjs');
  const databaseUrl = 'postgresql://user:password@db.example.test:5432/postgres';
  assert.deepEqual(poolConfigForDatabaseUrl(databaseUrl, {}), {
    connectionString: databaseUrl,
    ssl: { rejectUnauthorized: false },
    max: 1,
  });
  assert.deepEqual(poolConfigForDatabaseUrl(databaseUrl, { DATABASE_SSL_REJECT_UNAUTHORIZED: 'true' }).ssl, {
    rejectUnauthorized: true,
  });
  assert.equal(poolConfigForDatabaseUrl(`${databaseUrl}?sslmode=disable`, {}).ssl, undefined);
  assert.equal(poolConfigForDatabaseUrl(databaseUrl, { DATABASE_SSL: 'false' }).ssl, undefined);
});
