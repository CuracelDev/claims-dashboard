...
function countsFor(item, contexts, batches) {
  ...
  return {
    ...
    confirmed_piles: ledgerOr('confirmed_pile_count', sum(batches, 'confirmed_pile_count')),
    reconciliation_pending: ledgerOr('reconciliation_pending_pile_count', sum(batches, 'pending_pile_count')),
    conflicts: ledgerOr('conflict_pile_count', sum(batches, 'conflict_pile_count')),
    failed: ledgerOr('failed_pile_count', sum(batches, 'failed_pile_count')),
...
export function toRunnerProgressView(run = {}, insurerRuns = [], contexts = [], batches = [], workItems = [], { now = Date.now() } = {}) {
  ...
  const insurers = ownRuns.map((item) => publicInsurer(item,
    contexts.filter((row) => row.insurer_run_id === item.id), batches.filter((row) => row.insurer_run_id === item.id),
    ownWork.find((work) => work.covered_by_insurer_run_id === item.id), run.run_source, now));
