...
export function toRunnerProgressView(run = {}, insurerRuns = [], contexts = [], batches = [], workItems = [], { now = Date.now() } = {}) {
  ...
  if (run.mode === 'dry-run' && run.details?.preview_protocol === 'durable_preview_v1') {
    summary_text = status === 'completed'
      ? 'Preview completed; no assignments were submitted.'
      : status === 'failed'
        ? 'Preview failed; no assignments were submitted.'
        : 'Preview completed with issues; no assignments were submitted.';
  }
