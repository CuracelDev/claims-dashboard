...
const DISPOSITIONS = new Set(['queued', 'claimed', 'covered_by_active_cycle', 'follow_up_queued', 'inactive', 'completed', 'failed', 'cancelled', 'diagnostic_preview']);
...
const ERRORS = {
  ...
  assignment_plan_incomplete: 'At least one persisted plan did not reach portal submission after the bounded same-run retry.',
  invalid_configuration: 'Insurer configuration needs review.',
  no_eligible_assignees: 'No active, available assignee has valid capacity configuration; operator review is required.',
  ...
  probe_blocked_by_capacity: 'Read-only probe blocked by worker capacity.',
  preview_completed_with_issues: 'Preview completed with issues; no assignments were submitted.',
  preview_parent_ownership_lost: 'Preview ownership was lost before the parent could be finalized.',
...
const PREVIEW_SUMMARIES = {
  queued: 'Preview queued; no assignments have been submitted.',
  running: 'Preview running; no assignments are being submitted.',
  completed: 'Preview completed; no assignments were submitted.',
  completed_with_issues: 'Preview completed with issues; no assignments were submitted.',
  partial: 'Preview completed with issues; no assignments were submitted.',
  failed: 'Preview failed; no assignments were submitted.',
  manual_action_required: 'Preview requires manual action; no assignments were submitted.',
  cancelled: 'Preview cancelled; no assignments were submitted.',
  covered_by_active_cycle: 'Preview covered by an active cycle; no assignments were submitted.',
  skipped_overlap: 'Preview skipped for overlap; no assignments were submitted.',
  unknown: 'Preview status is unknown; no assignments are confirmed.',
};
...
function countsFor(item, contexts, batches) {
  ...
  // finished_at is written only when every attempt is terminal. The residual
  // after confirmed/conflict/failed attempts is therefore manual action, not
  // missing or pending work. This also exposes later historical reconciliation
  // without relying on the original insurer's terminal snapshot.
  const manualFromBatches = batches.reduce((total, batch) => {
    if (!timestamp(batch.finished_at) || count(batch.pending_pile_count)) return total;
    const disposed = count(batch.confirmed_pile_count) + count(batch.conflict_pile_count) + count(batch.failed_pile_count);
    return total + Math.max(0, count(batch.planned_pile_count) - disposed);
  }, 0);
  const plannedPiles = ledgerOr('planned_pile_count', sum(batches, 'planned_pile_count'));
  const submittedPiles = ACTIVE.has(item.status) && !count(item.submitted_pile_count)
    ? null
    : count(item.submitted_pile_count);
  return {
    ...
    planned_piles: plannedPiles,
    selected_piles: sum(batches, 'selected_pile_count'),
    submitted_piles: submittedPiles,
    unsubmitted_plans: submittedPiles == null ? null : Math.max(0, plannedPiles - submittedPiles),
    ...
    manual_action_required: manualFromBatches || (item.status === 'manual_action_required' || item.error_code === 'manual_action_required' ? null : 0),
  };
}
...
function previewInsurers(run, source, now) {
  const values = run.details?.preview_protocol === 'durable_preview_v1' ? run.details?.preview_outcomes : null;
  if (!Array.isArray(values) || values.length > 64) return [];
  return values.map((item, index) => {
    const syntheticId = `preview-${index + 1}`;
    const insurer = publicInsurer({
      id: syntheticId, insurer_name: item?.insurer_name,
      status: item?.status, phase: item?.phase, error_code: item?.error_code,
      discovered_pile_count: item?.discovered_piles, discovered_claim_count: item?.discovered_claims,
      planned_pile_count: item?.planned_piles, submitted_pile_count: 0,
      confirmed_pile_count: 0, reconciliation_pending_pile_count: 0,
      conflict_pile_count: 0, failed_pile_count: item?.status === 'failed' ? item?.planned_piles : 0,
      started_at: run.started_at, finished_at: run.finished_at,
    }, [], [],
    { source, disposition: 'diagnostic_preview' }, source, now);
    const contextKeys = ['total', 'complete', 'empty', 'failed', 'pending'];
    const measured = Object.fromEntries(contextKeys.map((key) => [key, item?.[`contexts_${key}`]]));
    const validContexts = contextKeys.every((key) => Number.isSafeInteger(measured[key]) && measured[key] >= 0 && measured[key] <= 100000)
      && measured.empty <= measured.complete
      && measured.complete + measured.failed + measured.pending === measured.total;
    const contextCounts = Object.fromEntries(contextKeys.map((key) => [
      `contexts_${key}`, validContexts ? measured[key] : null,
    ]));
    insurer.counts = {
      ...insurer.counts,
      ...contextCounts,
      discovered_piles: count(item?.discovered_piles),
      unassigned_piles: count(item?.discovered_piles),
      discovered_claims: count(item?.discovered_claims),
      planned_piles: count(item?.planned_piles),
    };
    return insurer;
  });
}
export function toRunnerProgressView(run = {}, insurerRuns = [], contexts = [], batches = [], workItems = [], { now = Date.now() } = {}) {
  ...
  if (!insurers.length) insurers.push(...previewInsurers(run, run.run_source, now));
  ...
  const activePreview = ACTIVE.has(status) && run.details?.preview_protocol === 'durable_preview_v1';
  const previewHeartbeatAt = activePreview ? timestamp(run.updated_at) : null;
  ...
  if (run.mode === 'dry-run' && run.details?.preview_protocol === 'durable_preview_v1') {
    summary_text = PREVIEW_SUMMARIES[status] || PREVIEW_SUMMARIES.unknown;
  }
  ...
  return {
    ...
    phase: active[0]?.phase || insurers.at(-1)?.phase || (activePreview ? member(run.details?.preview_phase, PHASES, 'configuration') : ACTIVE.has(status) ? 'configuration' : status === 'unknown' ? 'unknown' : 'complete'),
    heartbeat_at: insurers.map((item) => item.heartbeat_at).filter(Boolean).sort((a, b) => Date.parse(a) - Date.parse(b)).at(-1) || previewHeartbeatAt,
    heartbeat_state: status === 'unknown' ? 'unknown' : !ACTIVE.has(status) ? 'not_applicable'
      : activePreview && !active.length ? heartbeat({ heartbeat_at: previewHeartbeatAt }, status, now)
        : ['stale', 'unknown', 'missing', 'fresh'].find((state) => active.some((item) => item.heartbeat_state === state)) || 'missing',
    ...
    work_disposition: dispositions.size > 1 ? 'mixed' : [...dispositions][0]
      || (run.details?.preview_protocol === 'durable_preview_v1' ? 'diagnostic_preview'
        : request_state.state === 'waiting' ? 'waiting' : request_state.state === 'acknowledged' ? 'covered_by_active_cycle' : 'legacy'),
