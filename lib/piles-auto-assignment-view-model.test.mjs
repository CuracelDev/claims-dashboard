...
function historyRoute(tables, errors = {}) {
  const calls = [];
  const database = { from(table) {
    ...
    const query = {
      ...
      then(resolve) {
        ...
        for (const filter of call.filters) rows = rows.filter((row) => filter.kind === 'eq' ? row[filter.column] === filter.value : filter.value.includes(row[filter.column]));
