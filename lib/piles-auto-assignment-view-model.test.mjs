...
test('counts aggregate contexts and batches without claim identifiers', () => {
  ...
  assert.deepEqual(Object.fromEntries(Object.entries(view.counts).filter(([key]) => !['contexts_empty', 'contexts_failed', 'contexts_pending', 'selected_piles', 'submitted_piles', 'unsubmitted_plans', 'manual_action_required'].includes(key))), {
...
test('counts remain owned and expose empty/failed/pending contexts and selected/submitted/manual evidence', () => {
  ...
  assert.equal(view.counts.unsubmitted_plans, 2);
...
test('unavailable assignee configuration is explained as operator action', () => {
  const view = project(parent, [{
    ...child,
    status: 'completed_with_issues',
    error_code: 'no_eligible_assignees',
  }]);
  assert.match(view.insurers[0].error_message, /active.*available.*operator review/i);
});
...
test('GET loads bounded parent-owned safe work and timing columns and serializes only safe diagnostics', async () => {
  ...
  const batchQueries = fixture.calls.filter((call) => call.table === tables.batches);
  assert.ok(batchQueries.length);
  assert.ok(batchQueries.every((query) => query.columns.includes('finished_at')));
});
...
test('terminal batch residual exposes manual action without reading attempt identities', () => {
  const view = project(parent, [{ ...child, status: 'completed_with_issues' }], [], [{
    id: 'batch-1', insurer_run_id: 'i1', status: 'failed',
    finished_at: '2026-09-16T08:30:00+00:00', planned_pile_count: 4,
    confirmed_pile_count: 2, pending_pile_count: 0,
    conflict_pile_count: 1, failed_pile_count: 0,
  }]);
  assert.equal(view.counts.manual_action_required, 1);
  assert.equal(view.insurers[0].counts.manual_action_required, 1);
});
...
test('unsubmitted plan failures have a fixed safe explanation', () => {
  const view = project(parent, [{
    ...child,
    status: 'failed',
    error_code: 'assignment_plan_incomplete',
    error_message: '<html>private claim</html>',
    planned_pile_count: 2,
    submitted_pile_count: 0,
  }]);
  assert.equal(view.insurers[0].counts.unsubmitted_plans, 2);
  assert.match(view.insurers[0].error_message, /persisted plan.*portal submission/i);
  assert.doesNotMatch(view.insurers[0].error_message, /private claim/i);
});
