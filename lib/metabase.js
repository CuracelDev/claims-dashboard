 ...
 */
async function getDailyClaimsByInsurer({ startDate, endDate, skipCache = false } = {}) {
  // Default to last 30 days if no dates provided
  const end = endDate || new Date().toISOString().split('T')[0];
  const start = startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];

  const cacheKey = `daily_claims_${start}_${end}`;
  ...
  if (!skipCache) {
    ...
    if (cached) {
      ...
      if (cached.isStale) {
        ...
        fetchAndCacheDailyClaims(start, end, cacheKey).catch(err => 
  ...
  // Fetch fresh data
  return fetchAndCacheDailyClaims(start, end, cacheKey);
}

/**
 * Internal: fetch daily claims and update cache
 */
async function fetchAndCacheDailyClaims(start, end, cacheKey) {
  const sql = `
    SELECT 
    ...
    WHERE submitted_at IS NOT NULL
      ...
      AND submitted_at <= '${end} 23:59:59'
    GROUP BY DATE(submitted_at), hmo_id
