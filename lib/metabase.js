...
async function fetchAndCacheDailyClaims(start, end, dateField, cacheKey) {
  ...
  const whereClause = dateField === 'submitted_at' 
    ? `WHERE ${dateField} IS NOT NULL AND ${dateField} >= '${start}' AND ${dateField} <= '${end} 23:59:59'`
    : `WHERE ${dateField} >= '${start}' AND ${dateField} <= '${end} 23:59:59'`;
  ...
  const sql = `
    ...
    FROM claims
    ${whereClause}
    GROUP BY DATE(${dateField}), hmo_id
